...
Multi-Step Financial Calculations: Candidates are frequently tripped up by highly specific formulas (e.g., autoregressive AR(1) models, random walks, forward rate agreements, binomial trees) where knowing the final answer is useless without a clear, step-by-step mathematical derivation.
Highly Nuanced Ethical Rules: Standard I-VII of the CFA Institute Code of Ethics require candidates to evaluate complex professional scenarios where a tiny contextual detail completely changes whether an ethics violation has occurred.
The Solution
...
Ingestion Flow
Semantic Chunking: Text documents are split into optimal semantic chunks preserving math formulas and contextual integrity.
Vector Embedding: We utilize the high-performance OpenAI text-embedding-3-small model to generate semantic representations.
Vector Database: Chunks are indexed and persisted in a Chroma vector database (./chroma_db) for fast, low-latency semantic retrieval.
...
Mechanism: Single-shot retrieval. It fetches the top 
k
 chunks from the database, builds the prompt context, and executes the LLM in one pass. It is fast, cost-efficient, and ideal for straightforward queries.
...
Retrieval Hit Rate (HR): Measures if the correct reference document containing the ground-truth explanation is within the retrieved top-$k$ results.
Mean Reciprocal Rank (MRR): Measures the precision of retrieval, rewarding the system when the most relevant chunk is retrieved at Rank 1.
Accuracy / Verification Rate (LLM-as-a-judge): A strict evaluator model grades both outputs against the gold standard Expected Knowledge. Our code avoids fragile text matching by implementing a robust parsing structure that extracts clean grading verdicts.
📊 Automation Sweep Script (run_grid_search.sh)
You can run automated grid sweeps over different 
K
 values and retrieval algorithms:

./run_grid_search.sh
This automatically tests multiple combinations (e.g., 
K
=
3
 similarity vs 
...
=
5
 MMR), writes stdout logs to logs/, and saves final quantitative JSON experiment reports directly into the report/ directory.
...
Sync Dependencies & Launch Environment:
uv sync
Run the Interactive CLI Chat (with live SQLite Logging and color codes):
...
Execute Interactive Chat within Container:
